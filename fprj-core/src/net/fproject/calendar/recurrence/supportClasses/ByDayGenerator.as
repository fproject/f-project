package net.fproject.calendar.recurrence.supportClasses
{
	...
	/**
	 * 
	 * Instances of ByDayFilter are used to generate values in BYDAY recurrence rule parts
	 * 
	 */
	internal class ByDayGenerator implements IGenerator
    {
        ...
        function ByDayGenerator(calendar:GregorianCalendar, startDate:Date, days:Array, weeksInYear:Boolean)
        {
        ...
        private function generateDatesInMonth(y:int, m:int) : void
        {
            ...
            for each (var wd:RecurrenceWeekDay in this._days)
            {
                ...
                if (wd.modifier == 0)
                {
					var tmpDate:Date = new Date(ymFirst.time);
